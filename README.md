...
Можно задавать свой порог изменения цены (в процентах):

```bash
price-alert BTC --threshold 7
```

> Подробнее см. Issue #4.
## Features

- ✅ Лёгкий CLI (Python)
- ✅ Поддержка популярных тикеров (BTC/ETH/SOL)
- ✅ Конфигурируемый порог через `--threshold`
- ✅ Установка в editable-режиме: `pip install -e .`
- 🛠️ План: логирование и экспорт метрик

Подробнее см. обсуждение в Issue #6.
